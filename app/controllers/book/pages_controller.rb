class Book::PagesController < ApplicationController
  before_action :set_book_page, only: [:show, :edit, :update, :destroy]
  ...
  def create
    @book_page = Book::Page.new(book_page_params.merge(book: book)) #変更
  ...
  private
    ...
    def book
      @book ||= Book.find(params[:book_id]) #現在指している本が帰ってくる
    end

    helper_method :book  #htnlにアクセスできるようになる
